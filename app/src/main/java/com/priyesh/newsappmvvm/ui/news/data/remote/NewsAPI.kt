...
import com.priyesh.newsappmvvm.ui.news.data.model.NewsResponse
...
class NewsAPI @Inject constructor(private val requestHandler: RequestHandler) {
    ...
    suspend fun getTopHeadlines(category: String?, page: Int): NetworkResult<NewsResponse> {
        val map = if (!category.isNullOrEmpty()) mapOf(
            "country" to "us",
            "category" to category,
            "page" to page,
            "apiKey" to NetworkConstants.API_KEY
        ) else mapOf("country" to "us", "page" to page, "apiKey" to NetworkConstants.API_KEY)
